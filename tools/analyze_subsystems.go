...
func AnalyzeSubsystemsTool() *mcp.Tool {
	return &mcp.Tool{
		...
		Description: "Анализ распределения объектов конфигурации 1С по подсистемам. " +
			"action=orphans: объекты вне подсистем. Универсум охватывает ВЕСЬ набор видов верхнего уровня, пригодных для состава подсистемы: прикладные (справочники, документы, регистры, отчёты, обработки, планы, бизнес-процессы, задачи, перечисления) и служебные (константы, общие модули, общие формы, команды и группы команд, общие макеты и картинки, роли, определяемые типы, HTTP и web сервисы, пакеты XDTO, параметры сеанса, регламентные задания, функциональные опции и их параметры, подписки на события, журналы документов, хранилища настроек, стили и элементы стиля, критерии отбора, последовательности, общие реквизиты, нумераторы, WS ссылки, внешние источники данных), кроме самого контейнера (подсистемы) и языков (язык не входит в состав подсистемы). " +
