...
func TestRenderFailure_NoDashes(t *testing.T) {
	shipped := []string{
		...
		fmt.Sprintf(lineStatusExtension, 400),
		fmt.Sprintf(lineStatusDenial, 403),
		fmt.Sprintf(lineStatusForeign, 401, 1234),
		fmt.Sprintf(lineStatusForeignTruncated, 500),
		fmt.Sprintf(lineForeignContentType, "text/html"),
		...
		remedyEventLogEventFilterUnresolved,
		...
		renderFailure(headingEventLog, &onec.StatusError{StatusCode: 403, BodyKind: onec.BodyKindExtension, Detail: eventLogEventFilterRefusalPrefix}),
		renderFailure(headingEventLog, &onec.StatusError{StatusCode: 403, BodyKind: onec.BodyKindDenial, Detail: "env forbidden"}),
		renderFailure(headingQuery, &onec.StatusError{StatusCode: 402, BodyKind: onec.BodyKindDenial, Detail: "seat limit exceeded"}),
