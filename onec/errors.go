...
//
// ТО ЖЕ САМОЕ ВЕРНО ПРО КОНВЕРТ ОТКАЗА. Издание Advanced ходит к публикации не
// напрямую, а через шлюз, и отказ шлюза (нет доступа к среде, нужен повторный
// вход на проде, исчерпан лимит мест, превышена частота, вышестоящий узел
// недоступен) приходит тем же телом: объект JSON, у которого ключ error несёт
// код и текст. Пишет его НАША сторона, не веб-сервер и не прокси, поэтому довод
// о физических путях к нему не относится, а причина отказа есть весь смысл
// ответа. Значением ключа error там может быть не строка, а вложенный объект
// {"code","message"}; сам ключ тот же самый, то есть нового формата на проводе
// это не заводит (см. extensionEnvelopeDetail).
const (
	// BodyKindExtension — тело разобралось как конверт {"error":…}: либо конверт
	// расширения 1С, либо конверт отказа шлюза. Оба несут диагностику, которую
	// написала наша сторона, поэтому класс у них один.
	//
	// ЧЕГО ЭТОТ КЛАСС НЕ РАЗЛИЧАЕТ. Кто именно написал текст, поле не говорит, и
	// отрисовщик отказа в tools называет источником 1С (lineStatusExtension,
	// captionOnecError). Для отказа шлюза это неточно: ответил шлюз перед
	// публикацией. Разделение оставлено несделанным НАМЕРЕННО: в самом издании
	// Community шлюза нет вовсе, его собственные производители конверта дают
	// только строковую форму (extension/src/HTTPServices/MCPService/Ext/Module.bsl
	// строит Новый Структура("error", ТекстОшибки), cmd/mock-1c — map[string]string),
	// поэтому третий класс сделал бы формулировку общего и частого случая
	// расплывчатой ради случая, которого здесь не бывает. Свойство, ради которого
	// текст обрамляют, от источника не зависит и остаётся верным: untrustedTextNotice
	// говорит, что ниже данные, а не инструкция.
...
type StatusError struct {
	...
	BodyKind string
	// Detail — диагностика из конверта {"error":…}. Заполняется ТОЛЬКО при
	// BodyKind == BodyKindExtension. Для строковой формы это значение ключа
	// целиком, для вложенной формы отказа — «код: текст» (или одно из двух, если
	// пришло одно).
	Detail string
...
//
// ДВЕ ФОРМЫ, ОДИН КЛЮЧ. Строковая форма {"error":"текст"} это то, что строит
// расширение 1С. Вложенная форма {"error":{"code":…,"message":…}} это отказ
// шлюза, через который ходит издание Advanced. Ключ у них общий, поэтому
// вложенная форма разбирается тем же предикатом, а не новым форматом на проводе.
//
...
func extensionEnvelopeDetail(body []byte) (string, bool) {
	...
	if s, ok := nonBlankJSONString(raw); ok {
		return s, true
	}
	return denialEnvelopeDetail(raw)
}
