...
var ErrURLCredentialUnstrippable = errors.New(
	"адрес 1С содержит логин и пароль в форме, которую невозможно разобрать однозначно, поэтому он отклонён. " +
		...
		"Если они обязаны остаться в адресе, закодируйте служебные символы: @ как %40, / как %2F, ? как %3F, # как %23")
...
var ErrBaseURLAmbiguousAt = errors.New(
	"адрес 1С читается двояко и поэтому отклонён: в его пути есть знак @, а перед ним есть двоеточие, " +
		"то есть всё, что стоит до @, читается и как часть адреса, и как логин с паролем. " +
