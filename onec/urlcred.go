...
func SplitURLCredentials(raw string) (BaseURLSplit, error) {
	...
	// Шаг 6. Обязательство доказать результат, а не предположить его: адрес
	// разбирается ЗАНОВО, и в нём не должно остаться ни userinfo, ни другого
	// хоста.
	//
	...
	// отклоняла собственный корректный результат. Повторный разбор смотрит на
	// то, что здесь и требуется: authority.
	back, backErr := url.Parse(base)
	if backErr != nil || back.User != nil || back.Host != parsed.Host {
		return BaseURLSplit{}, ErrURLCredentialUnstrippable
	}
