...
func main() {
	...
	stripDefaultRoles := flag.Bool("strip-default-roles", false, "Install mode: do not declare MCP_ОсновнаяРоль as a default role of the configuration. For a Standard Subsystems base whose session abort names «или указаны лишние роли»: that clause marks a library whose check counts ОсновныеРоли, and removing our entry can return the count to an accepted value, but only if ours is the only extra one. A library that checks only that АдминистраторСистемы and ПолныеПрава are present ignores the count, and there this flag fixes nothing. Not verified end to end: we have never seen this flag remove the abort on a base that reproduces it, because no base we can reach reproduces it. The count rule was read from a public copy of the library, not measured on ours; what we measured on ours is that there is no count test at all and a fourth entry is accepted. Cost, measured: an ordinary least-privileged account loses the service until it is granted access explicitly, while an administrator keeps access and notices nothing. Where the configuration has the Управление доступом subsystem, grant it through a профиль групп доступа: a recalculation of user roles deletes a role assigned to a user directly and switches off the property carrying the automatic access.")
