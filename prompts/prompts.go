...
2. Если доступен инструмент search_code (сервер запущен с --dump), используй его чтобы найти код модулей этого объекта; иначе пропусти этот шаг
...
2. Если доступен инструмент search_code (сервер запущен с --dump), используй его чтобы найти текущий код модуля объекта; иначе пропусти этот шаг
...
3. Если ошибка связана с кодом модуля и доступен инструмент search_code (сервер запущен с --dump) -- используй его для поиска исходного кода
...
Шаги:
1. Используй инструмент get_object_structure чтобы посмотреть структуру объекта (object_type: %s, object_name: %s)
2. Если доступен инструмент search_code (сервер запущен с --dump), используй его чтобы найти код модулей объекта; иначе пропусти этот шаг
3. Проанализируй код на:
...
## Маппинг категорий метаданных на имена таблиц запросов и object_type

- Справочники -> Справочник.X (object_type для get_object_structure: Catalog)
- Документы -> Документ.X (object_type: Document)
- РегистрыНакопления -> РегистрНакопления.X (object_type: AccumulationRegister)
- РегистрыСведений -> РегистрСведений.X (object_type: InformationRegister)
- РегистрыБухгалтерии -> РегистрБухгалтерии.X (object_type: AccountingRegister)
- ПланыСчетов -> ПланСчетов.X (object_type: ChartOfAccounts)
- ПланыВидовХарактеристик -> ПланВидовХарактеристик.X (object_type: ChartOfCharacteristicTypes)
- Перечисления -> НЕ являются таблицами (используй ЗНАЧЕНИЕ()), object_type: Enum
...
- search_code (только если сервер запущен с --dump): поиск по ключевым словам в коде модулей (найти логику обработки, вычисления)
...
- search_code (только если сервер запущен с --dump): для поиска логики в коде (как что-то вычисляется, где обрабатывается)
...
2. Найди релевантные объекты: вызови get_metadata_tree с фильтром по нужной категории; если доступен инструмент search_code (сервер запущен с --dump), также выполни поиск по ключевым словам задачи
3. Изучи структуру объектов: вызови get_object_structure для каждого найденного объекта, запомни точные имена реквизитов
4. Найди существующий код: если доступен инструмент search_code (сервер запущен с --dump), вызови его по именам процедур и объектов и изучи текущую реализацию; иначе пропусти этот шаг
...
- Проверил ли существующий код в этих модулях? (если нет и сервер запущен с --dump: search_code)
