...
> [!TIP]
> **Топ-5 бета-тестеров** получают бесплатную подписку навсегда на Профессиональную версию. Подробности и условия в [docs/advanced.md](docs/advanced.md)
...
- **4 инструмента для работы с живой базой.** Метаданные, информация о конфигурации, формы, запросы к данным (с параметрами), поиск по коду, валидация, журнал регистрации, справка BSL.
- **Полнотекстовый поиск по коду** (`code_search`). Три режима: smart (BM25-ранжирование), regex, exact. Встроенные BSL-синонимы — поиск по `StrFind` находит `СтрНайти` и наоборот.
...
| Инструмент | Действие (action) | Описание |
|------------|-------------------|---------|
| `code_read` | `metadata_tree` | Дерево метаданных: справочники, документы, регистры, общие модули и др. |
| `code_read` | `object_structure` | Реквизиты, табличные части, измерения и ресурсы конкретного объекта |
| `code_read` | `form_structure` | Структура основной формы: элементы, команды, обработчики событий |
| `code_read` | `config_info` | Имя конфигурации, версия, поставщик, версия платформы, режим работы |
| `code_search` | `text` | Полнотекстовый поиск по коду модулей: smart (BM25), regex, exact. BSL-синонимы (рус↔англ). Фильтрация по типу метаданных и модуля |
| `code_search` | `syntax_help` | Справка по 180 встроенным функциям, методам типов и паттернам BSL |
| `code_execute` | `query` | Выполнить запрос на языке запросов 1С с параметрами (только SELECT/ВЫБРАТЬ) |
| `code_execute` | `validate` | Проверить синтаксис запроса без выполнения |
| `system` | `event_log` | Чтение журнала регистрации с фильтрацией по дате, уровню и пользователю |
...
| `--dump` | — | — | Путь к выгрузке конфигурации (DumpConfigToFiles), включает инструмент code_search |
